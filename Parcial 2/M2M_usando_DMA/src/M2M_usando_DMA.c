...
1-Usar el hardware GPDMA para transferir los datos en palabras de 2 bytes desde la primera mitad de la SRAM del banco 1 hacia la segunda mitad de la SRAM del banco 0
...
#define bank0_finish 0x2007E000 // segunda mitad banco 0
...
void config_gpdma(void)
{
    ...
    dmaCfg.channelNum 	= GPDMA_CH_0;  		// Canal menos prioritario
    dmaCfg.transferSize = 0x0FFF;			// 4096 --> 0000 1111 1111 1111
    dmaCfg.type 		= GPDMA_M2M; 		// tranferencia MEM->MEM
    dmaCfg.srcMemAddr 	= (uint32_t) &(bank1_start);
    dmaCfg.dstMemAddr 	= (uint32_t) &(bank0_finish);
	dmaCfg.srcConn 		= DISABLE;
	dmaCfg.dstConn		= DISABLE;
    dmaCfg.src 			= srcc;
    dmaCfg.dst 			= dstc;
    dmaCfg.intTC 		= DISABLE;
    dmaCfg.intErr 		= DISABLE;
    dmaCfg.linkedList	= DISABLE;
